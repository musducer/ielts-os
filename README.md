...
- A transient AI-provider timeout is reported as `FAILED / GENERATION_TIMEOUT` with `NOT_RUN` round-trip state, never as a DOCX parser failure. The batch panel offers `Retry AI` only for those provider failures; genuine source, media, validation and parser-contract reviews remain fail-closed.
