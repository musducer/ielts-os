...
- Canonical JSON decoding is inside that same provider retry envelope. A malformed model response may consume the remaining bounded provider attempt, but a successfully decoded payload must still pass strict `CanonicalExam.from_dict`, grounding, media, rendering and parser round-trip gates. Never syntax-repair, auto-fill or silently drop canonical fields after decoding.
