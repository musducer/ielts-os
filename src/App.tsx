...
// ==========================================
      ...
      if (activeExam) {
          ...
          const setCleanDragImage = (event: any, label: string) => {
              ...
              ghost.style.cssText = 'position:fixed;left:-10000px;top:-10000px;max-width:360px;padding:8px 12px;background:#fff;border:1px solid #9ca3af;border-radius:4px;color:#111827;font:600 13px Arial,sans-serif;line-height:1.35;box-shadow:0 3px 12px rgba(0,0,0,.16);pointer-events:none;';
                  ...
                  {(() => {
                      ...
                      return visibleGroups.map((group) => {
                          ...
                          const safeHtml = (() => {
                              ...
                              // Word often wraps an arrow in nested divs. Match the standalone text node itself,
                              // rather than assuming the arrow's div is the outermost element.
                              out = out.replace(new RegExp('>\\s*' + flowArrowToken + '\\s*<', 'gi'), '>' + flowArrowHtml + '<');
                      ...
                      {vocabView === 'game' ? (() => {
                          ...
                          <VocabBlitz cards={vocabCards} C={C} awardsLeft={_awardsLeft}
                              ...
                              onAward={(n: number) => {
                                  ...
                                  const coinOperation = makeCoinOperation(me.id, n, "VOCAB_GAME_REWARD");
                                  setStudents(nx as any); syncData({ students: nx, __coinOperation: coinOperation });
                              }} />
                          );
                      })() : vocabView === 'study' ? (
                          dueCards.length === 0 ? (
