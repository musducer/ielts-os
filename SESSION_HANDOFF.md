...
- The raw-DOCX provider outer timeout must not be shorter than the backend provider's own 55-second request/fallback window. The pipeline now bounds generation to two 90-second attempts. If it still times out, mark the row `FAILED / GENERATION_TIMEOUT / NOT_RUN`, preserve its source, and expose the retry route; never report a non-existent DOCX as a parser-contract failure.
